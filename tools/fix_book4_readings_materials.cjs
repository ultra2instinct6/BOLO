...
if (!Array.isArray(context.BOOK4_CUSTOM_STORIES)) {
  throw new Error('BOOK4_CUSTOM_STORIES not found in readings.js');
}
...
function tokenize(text) {
  return (String(text || '').match(/[A-Za-z]+(?:'[A-Za-z]+)?/g) || []).map((word) => word.toLowerCase());
}
...
function ensureVerbs(story) {
  ...
  const verbs = story.partsOfGrammar.verbs;
  const seen = new Set(verbs.map((entry) => normalizeWord(entry && entry.en)).filter(Boolean));
  ...
  let added = 0;
  const tokens = tokenize(story.englishStory || story.english || '');
...
const stories = context.BOOK4_CUSTOM_STORIES;
let verbsStoriesUpdated = 0;

for (const story of stories) {
  const beforeCount = Array.isArray(story?.partsOfGrammar?.verbs) ? story.partsOfGrammar.verbs.length : 0;
  ensureVerbs(story);
  const afterCount = Array.isArray(story?.partsOfGrammar?.verbs) ? story.partsOfGrammar.verbs.length : 0;
  if (afterCount !== beforeCount) {
    verbsStoriesUpdated += 1;
  }
}

const replacement = `var BOOK4_CUSTOM_STORIES = ${JSON.stringify(stories, null, 2)};`;
const blockRegex = /var BOOK4_CUSTOM_STORIES = \[[\s\S]*?\n\];/;
if (!blockRegex.test(source)) {
  throw new Error('BOOK4_CUSTOM_STORIES block not found in readings.js');
}
