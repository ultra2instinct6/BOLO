...
if (!Array.isArray(context.BOOK9_CUSTOM_STORIES)) {
  throw new Error('BOOK9_CUSTOM_STORIES not found in readings.js');
}
...
const stories = context.BOOK9_CUSTOM_STORIES;
let verbsTouched = 0;
...
const replacement = `var BOOK9_CUSTOM_STORIES = ${JSON.stringify(stories, null, 2)};`;
const blockRegex = /var BOOK9_CUSTOM_STORIES = \[[\s\S]*?\n\];/;
if (!blockRegex.test(source)) {
  throw new Error('BOOK9_CUSTOM_STORIES block not found in readings.js');
}
