...
if (!Array.isArray(context.BOOK1_CUSTOM_STORIES)) {
  throw new Error('BOOK1_CUSTOM_STORIES not found in readings.js');
}
...
function ensureVerbs(story) {
  ...
  for (const token of tokens) {
    if (verbs.length + additions.length >= MIN_VERBS) break;
    if (!COMMON_VERBS.has(token)) continue;
    if (seen.has(token)) continue;
    seen.add(token);
    additions.push({ en: token, pa: 'ਕਿਰਿਆ' });
  }
...
const stories = context.BOOK1_CUSTOM_STORIES;
let vocabTouched = 0;
let verbsTouched = 0;
...
const replacement = `var BOOK1_CUSTOM_STORIES = ${JSON.stringify(stories, null, 2)};`;
const blockRegex = /var BOOK1_CUSTOM_STORIES = \[[\s\S]*?\n\];/;
if (!blockRegex.test(source)) {
  throw new Error('BOOK1_CUSTOM_STORIES block not found in readings.js');
}
