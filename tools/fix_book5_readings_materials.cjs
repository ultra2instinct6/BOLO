...
if (!Array.isArray(context.BOOK5_CUSTOM_STORIES)) {
  throw new Error('BOOK5_CUSTOM_STORIES not found in readings.js');
}
...
const COMMON_VERBS = new Set([
  ...
  'use', 'uses', 'make', 'makes',
  'save', 'saves', 'cook', 'cooks',
  'plant', 'plants', 'share', 'shares'
]);
...
const stories = context.BOOK5_CUSTOM_STORIES;
let verbsTouched = 0;
...
const replacement = `var BOOK5_CUSTOM_STORIES = ${JSON.stringify(stories, null, 2)};`;
const blockRegex = /var BOOK5_CUSTOM_STORIES = \[[\s\S]*?\n\];/;
if (!blockRegex.test(source)) {
  throw new Error('BOOK5_CUSTOM_STORIES block not found in readings.js');
}
